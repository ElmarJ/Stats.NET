 ...
 */
public class PcSearch implements GraphSearch, SemLearningMethod {
...
//                }
	...
	public PatternWithParameters run(RectangularDataSet dataSet) {
		...
		PatternWithParameters estimatedGraph = null;
		
		try{
			PatternToDagSearch search = new PatternToDagSearch(new Pattern(pattern));
			Dag patDag = search.patternToDagMeekRules();				//turn into a DAG
...
//			System.out.println("343: after, TestCats.edgesToEvaluateCoeffs = " + TestCats.edgesToEvaluateCoeffs);
			...
			estimatedGraph = new PatternWithParameters(semImEstDag,pattern);
			
			if (estimatedGraph.weightHash==null){
