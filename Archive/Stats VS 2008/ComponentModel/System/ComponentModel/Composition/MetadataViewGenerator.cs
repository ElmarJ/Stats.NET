    ...
    internal static class MetadataViewGenerator
    {
...
#if !SILVERLIGHT
        private static ConstructorInfo _mdvInvalidOperationExceptionConstructor =
        ...
        private static Type GenerateInterfaceViewProxyType(Type viewType)
        {
            ...
            foreach (PropertyInfo propertyInfo in viewType.GetAllProperties())
            {
                ...
                {
                    // Generate: throw new InvalidOperationException("Code path should never be reached!");
                    proxyCtorIL.LoadValue("Code path should never be reached!");
                    proxyCtorIL.Emit(OpCodes.Newobj, _mdvInvalidOperationExceptionConstructor);
