...
namespace System.ComponentModel.Composition.Hosting
{
    ...
    public partial class DirectoryCatalog : ComposablePartCatalog, INotifyComposablePartCatalogChanged, ICompositionElement
    {
        ...
        public void Refresh()
        {
            ...
            // Take snapshot of changed definitions before state update.
            ComposablePartDefinition[] definitions = catalogsToAdd.SelectMany(cat => cat.Item2.Parts)
                .Concat(catalogsToRemove.SelectMany(cat => cat.Item2.Parts))
                .ToArray<ComposablePartDefinition>();

            // Fire change notification
            if (definitions.Length > 0)
            {
                this.OnChanged(new ComposablePartCatalogChangedEventArgs(definitions));
            }
        }
        ...
        private void OnChangedDoNothing(object sender, ComposablePartCatalogChangedEventArgs e)
        {
            // Do Nothing because we don't care about the notifications coming from the 
            // ComposablePartCatalogCollection and only have this method because it requires it.
        }
        ...
        private void Initialize(string path, string searchPattern)
        {
            ...
            this._catalogCollection = new ComposablePartCatalogCollection(null, OnChangedDoNothing);
