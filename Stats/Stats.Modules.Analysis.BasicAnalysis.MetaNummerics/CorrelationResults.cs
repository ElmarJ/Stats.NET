...
namespace Stats.Modules.Analysis
{
    public class CorrelationResults: Results
    {
        ...
        private TableElement buildTable()
        {
            ...
            int rowId = 0;
            foreach (KeyValuePair<IVariable<IObservation>, Dictionary<IVariable<IObservation>, double>> kvp in this.correlations)
            {
                table.Rows[rowId].Header = kvp.Key.Name;
                table.Columns[rowId].Header = kvp.Key.Name;

                int colId = 0;
                foreach (KeyValuePair<IVariable<IObservation>, double> kvp2 in kvp.Value)
                {
