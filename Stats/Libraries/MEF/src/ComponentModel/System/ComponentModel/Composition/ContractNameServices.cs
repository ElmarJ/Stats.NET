...
namespace System.ComponentModel.Composition
{
    internal static class ContractNameServices
    {
        ...
        [ThreadStatic]
        private static Dictionary<Type, string> contractNameCache;

        private static Dictionary<Type, string> ContractNameCache
        {
            get
            {
                return contractNameCache = contractNameCache ?? new Dictionary<Type, string>();
            }
        }

        internal static string GetDefaultContractName(Type type)
        {
            Assumes.NotNull(type);

            if (type.IsSubclassOf(typeof(Delegate)) && !type.IsAbstract)
            {
                MethodInfo method = type.GetMethod("Invoke");
                return GetContractNameFromMethod(method);
            }

            string typeName;
            if (!ContractNameCache.TryGetValue(type, out typeName))
            {
                StringBuilder typeNameStringBuilder = new StringBuilder();
                WriteTypeWithNamespace(typeNameStringBuilder, type);
                typeName = typeNameStringBuilder.ToString();
                ContractNameCache.Add(type, typeName);
            }
            return typeName;
        }

        internal static string GetContractNameFromMethod(MethodInfo method)
        {
