...
public class RegressionDataset implements Regression {
    ...
    public RegressionResult regress(Node target, List<Node> regressors) {
        ...
        DoubleMatrix2D xSub = data.subsetColumns(regressors).getDoubleData();
        ...
        DoubleMatrix2D xTxInv = new Algebra().mult(xT, x);

        xTxInv = new Algebra().inverse(xTxInv);
        DoubleMatrix2D pr2 = new Algebra().mult(xT, y);
        DoubleMatrix2D b = new Algebra().mult(xTxInv, pr2);
        ...
        String summary = createResultString(n, k, rss, r2, x, regressors, b, sqErr, t, p);
        ...
        return new RegressionResult(false, vNames, n,
                bArray, tArray, pArray, seArray, r2, rss, alpha);
    }
