...
public class DataStandardizer extends DataWrapper {
    ...
    public DataStandardizer(DataWrapper wrapper) {
        ...
        DoubleMatrix2D data2 = standardizeData(dataSet.getDoubleData());
        List<Node> list = dataSet.getVariables();
        List<ContinuousVariable> list2 = new ArrayList<ContinuousVariable>();

        for (Node node: list) {
            list2.add((ContinuousVariable) node);
        }
