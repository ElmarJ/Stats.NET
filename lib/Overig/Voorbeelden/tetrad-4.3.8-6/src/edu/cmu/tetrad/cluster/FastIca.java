 ...
 * <p/>
 * Performance. The R version scales up much better than this one does, the
 * main reason for which is that the calculation of the initial covariance matrix
...
public class FastIca {
    ...
    public IcaResult findComponents() {
        ...
        // Try reducing time to calculate XX' by sampling... (Not that great
        // actually... tolerances change a lot.)
//        DoubleMatrix2D V = new DenseDoubleMatrix2D(p, p);
//        int[] r = new int[n];
//
//        for (int i = 0; i < r.length; i++) {
//            r[i] = PersistentRandomUtil.getInstance().nextInt(n);
//        }
//
//        for (int i = 0; i < p; i++) {
...
//
//                for (int k = 0; k < n; k++) {
//                    sum += X.get(i, r[k]) * X.get(j, r[k]);
//                    double _ratio = sum / (k + 1);
//
//                    if (Math.abs(ratio - _ratio) < 1e-7) {
//                        break;
//                    }
//
//                    ratio = _ratio;
...
//        DoubleMatrix2D V = new DenseDoubleMatrix2D(p, p);
//        X.zMult(X.viewDice(), V);

//        System.out.println("Starting");
//        long time0 = System.currentTimeMillis();

//        // Just multiply it out by hand.
//        DoubleMatrix2D V = new DenseDoubleMatrix2D(p, p);
//
//        for (int i = 0; i < p; i++) {
...
//        System.out.println("Elapsed time: " + (time1 - time0) / 1000L + "s");

        // Trying to do multiplication using BLAS--BLAS takes 90 seconds;
        // translating back and forth into array[] takes another 120 seconds.
//        System.out.println("Starting");
//        long time0 = System.currentTimeMillis();
//
//        int m = X.rows();
...
//
//        Dgemm.dgemm("N", "T", m, m, k, 1.0, x, 0, m, x, 0, m, 0.0, z, 0, m);
////        print("Z", Z);
//
//        DoubleMatrix2D V = new DenseDoubleMatrix2D(m, m);
//
//        index = -1;
//        for (int j = 0; j < m; j++) {
//            for (int i = 0; i < m; i++) {
