...
import edu.cmu.tetrad.search.PatternWithParameters;
import edu.cmu.tetrad.search.Shimizu2006Search;
 ...
 *          2006) $
 */
public class TestLnn extends TestCase {

    /**
     * Standard constructor for JUnit test cases.
     */
    public TestLnn(String name) {
        super(name);
    }
    ...
    public void testLnn() {
        ...
        PatternWithParameters result = Shimizu2006Search.lingamDiscovery_DAG(data, .5);
        ...
        for (Parameter parameter : semPm.getParameters()) {
            if (parameter.getType() == ParamType.COEF) {
                ...
                double weight = result.weightHash.get(edge) ;
        ...
        try {
            ...
            DataSavers.saveContinuousData(data, out, ',');
            out.flush();
            out.close();
        } catch (IOException e) {
            e.printStackTrace();
...
//        System.out.println(pm.getDistributionParameters(x1));
        ...
        PatternWithParameters pwp = new PatternWithParameters(im, new Dag(graph));

        DoubleMatrix2D matrix = TestFastIca.simulate(pwp, 1000, new BetaDistribution(0.2, 0.3));

        List<ContinuousVariable> variables = new ArrayList<ContinuousVariable>();
    ...
    public static Test suite() {
        ...
        // of this class.
        return new TestSuite(TestLnn.class);
    }
}
