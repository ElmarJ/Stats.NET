...
namespace Shell
{
    ...
    public partial class MainShell : Form, IPartImportsSatisfiedNotification
    {
        ...
        {
            Export<HostSurfaceFactory, IDesignerMetadataView> exportSurfaceFactory = fileNewDialog.GetHostFactory();
            HostControl hc = new HostControl();
            if (designerCounts.ContainsKey(exportSurfaceFactory.MetadataView.ItemType))
                designerCounts[exportSurfaceFactory.MetadataView.ItemType]++;
            else
                designerCounts.Add(exportSurfaceFactory.MetadataView.ItemType, 1);
            string siteName = exportSurfaceFactory.MetadataView.ItemType + designerCounts[exportSurfaceFactory.MetadataView.ItemType].ToString();
            HostSurface hostSurface = exportSurfaceFactory.GetExportedObject().CreateNew(siteName);
            hc.InitializeHost(hostSurface);
            string fileName = siteName + "." + exportSurfaceFactory.MetadataView.FileExtension;
            TabPage tabpage = new TabPage(fileName + " - Design");
            tabpage.Tag = exportSurfaceFactory.MetadataView.Language;
            hc.Parent = tabpage;
        ...
        private void SetupMenus(HostSurface hostSurface)
        {
            ...
            if (hostSurface == null)
                return;
            IEnumerable<Export<Action<HostSurface>, ICommandMetadataView>> commands = hostSurface.HostSurfaceFactory.GetCommands();
            foreach (var command in commands)
            {
                ...
                {
                    if (mi.Text == command.MetadataView.Category)
                    {
                        found = true;
                        topMenu = mi;
                    }
                ...
                if(!found)
                {
                    topMenu = new MenuItem(command.MetadataView.Category);
                    this.mainMenu1.MenuItems.Add(topMenu);
                    menuItems.Add(topMenu);
                }
                MenuItem realItem = new MenuItem(command.MetadataView.Name);
                topMenu.MenuItems.Add(realItem);
                menuItems.Add(realItem);
                realItem.Tag = new CommandAction(command.GetExportedObject(), hostSurface);
