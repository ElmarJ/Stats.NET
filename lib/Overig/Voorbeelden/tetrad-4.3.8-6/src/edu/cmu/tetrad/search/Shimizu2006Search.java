...
public class Shimizu2006Search implements SemLearningMethod {
	...
	static PatternWithParameters lastShimizuGraph = null;
...
//		System.out.println("containsAllZeros: returning true");
    ...
    public static PatternWithParameters makeDagWithParms(ColtDataSet ltDataSet) {
    	...
    	for (int i=0; i<ltMat.rows(); i++)
    		for(int j=0; j<i; j++){
    			if (ltMat.get(i, j)!=0){
    				...
    				dwp.graph.addEdge(edge);

    				dwp.weightHash.put(edge,ltMat.get(i, j));
    			}
    		}
	...
	public PatternWithParameters lingamDiscovery_DAG(RectangularDataSet dataSet) {
 ...
 */
	public static PatternWithParameters lingamDiscovery_DAG(RectangularDataSet dataSet, double alpha) {

		//normalize data set
//		dataSet = normalizeVariance(dataSet);
		
		ColtDataSet Bhat = Shimizu2006Search.lingamDiscovery_Bhat(dataSet);
...
//		System.out.println("after pruning, Bhat = " + Bhat);
		...
		ColtDataSet B = Shimizu2006Search.lingamDiscovery_step5(Bhat); //B is the lower-triangular version of Bhat
		...
		PatternWithParameters icaDag = makeDagWithParms(B); //icaDag is the DAG made from Bhat's nonzero entries

		lastShimizuGraph = icaDag;
		isGraphUsed = false;
	...
	private static void pruneEdgesBySampling(ColtDataSet B, RectangularDataSet dataSet,
		...
		for (int i=0; i<nSamples; i++){
			RectangularDataSet currentBootstrapSample = isBootstrapSampling ?
					getBootstrapSample(dataSet) : //randomly draw from the dataSet, with replacement
...
//                        System.out.println("i="+i);
			...
			bHats[i] = lingamDiscovery_Bhat(currentBootstrapSample);
...
//					System.out.println("entries: "+printo);
					...
					B.getDoubleData().set(xi, xj, 0); //set it to zero
				}
			}
	}
...
//		System.out.println("bootstrapSample = " + bootstrapSample);
		return makeDataSet(bootstrapSample, dataSet.getVariables());
	}
...
//	}
    ...
    public static ColtDataSet makeDataSet(DoubleMatrix2D inVectors, List<Node> nodes) {
        ...
        Vector<ContinuousVariable> variables = new Vector<ContinuousVariable>();
        for (Node node : nodes)
