    ...
    public class AdvancedValueComposition
    {
        ...
        public void FunctionsFieldsAndProperties2_WithCatalog()
        {
            ...
            foreach (var export in c.opInfo)
            {
                ...
                {
                    Assert.AreEqual(3, export.GetExportedObject()(1, 2), "1 + 2 == 3");
                }
                else if ((string)export.Metadata["Var1"] == "sub")
                {
                    Assert.AreEqual(-1, export.GetExportedObject()(1, 2), "1 - 2 == -1");
                }
                else
                {
        ...
        public void FunctionsFieldsAndProperties2_StronglyTypedMetadata()
        {
            ...
            {
                if (export.MetadataView.Var1 == "add")
                {
                    Assert.AreEqual(3, export.GetExportedObject()(1, 2), "1 + 2 == 3");
                }
                else if (export.MetadataView.Var1 == "sub")
                {
                    Assert.AreEqual(-1, export.GetExportedObject()(1, 2), "1 - 2 == -1");
                }
                else
        ...
        public void InAdditionToCatalogTest()
        {
            ...
            foreach (var export in exports)
            {
                if (export.MetadataView.Var1 == "mult")
                {
                    Assert.AreEqual(2, export.GetExportedObject()(1, 2), "1 * 2 == 2");
                }
                else if (export.MetadataView.Var1 == "add")
                {
                    Assert.AreEqual(3, export.GetExportedObject()(1, 2), "1 + 2 == 3");
                }
                else if (export.MetadataView.Var1 == "sub")
        ...
        public void CollectionMetadataPropertyTest()
        {
            ...
            Assert.IsNotNull(export.MetadataView, "Should have metadata");
            Assert.IsNotNull(export.MetadataView.Values, "MetadataView should have collection of values");
            Assert.AreEqual(export.MetadataView.Values.Count(), 3, "Should have 3 elements");
            Assert.AreEqual(export.MetadataView.Values.First(), "One", "First should be 'One'");
            Assert.AreEqual(export.MetadataView.Values.Skip(1).First(), "two", "First should be 'two'");
            Assert.AreEqual(export.MetadataView.Values.Skip(2).First(), "3", "First should be '3'");
        }
#endif //!SIVLERLIGHT
        ...
        public void ImportExportSansNameTest()
        {
			...
			UnnamedImportAndExport unnamed = container.GetExportedObject<UnnamedImportAndExport>();
        ...
        public void MultipleInstantiationOfStaticCatalogItem()
        {
            ...
            StaticExport first = unnamedVI.GetExportedObject();
            StaticExport second = unnamedVI.GetExportedObject();
        ...
        public void MultipleInstantiationOfNonStaticCatalogItem()
        {
            ...
            NonStaticExport first = export1.GetExportedObject();
            NonStaticExport second = export2.GetExportedObject();
        ...
        public void ImportIntoUntypedExportTest()
        {
            ...
            CompositionBatch batch = new CompositionBatch();
            batch.AddExportedObject("untyped", 42);
            ...
            Assert.AreEqual(42, u.Export.GetExportedObject());

            var us = new UntypedExportsImporter();
            batch = new CompositionBatch();
            batch.AddExportedObject("untyped", 19);
            batch.RemovePart(rb);
            batch.AddPart(us);
            container.Compose(batch);
        ...
        public void ImportIntoDerivationOfExportException()
        {
            ...
            CompositionBatch batch = new CompositionBatch();
            batch.AddExportedObject("derived", typeof(DerivedExport), 42);
