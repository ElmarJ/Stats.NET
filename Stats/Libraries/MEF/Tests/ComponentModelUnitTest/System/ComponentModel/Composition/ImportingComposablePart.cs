...
namespace System.ComponentModel.Composition
{
    internal class ImportingComposablePart : ComposablePart
    {
        ...
        {
            var exportedObjects = exports.Select(export => export.GetExportedObject());

            int count = exportedObjects.Count();
            if (count == 0)
            {
                return null;
            }
            ...
            {
                return exportedObjects.First();
            }

            return exportedObjects.ToArray();
        }
    }
