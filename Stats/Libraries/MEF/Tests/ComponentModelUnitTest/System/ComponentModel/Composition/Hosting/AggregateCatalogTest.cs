...
namespace System.ComponentModel.Composition.Hosting
{
    ...
    public class AggregateCatalogTest
    {
        ...
        public void MutableCatalogNotifications()
        {
            ...
            // Add notifications
            catalog.Changed += delegate(object source, ComposablePartCatalogChangedEventArgs args)
            {
                // Local code
                ++step; ++step;
                changedStep = step;
...
#if !SILVERLIGHT
            var dirPartCatalog1 = new DirectoryCatalog(".");
            var dirPartCatalog2 = new DirectoryCatalog(".");
            var dirPartCatalog3 = new DirectoryCatalog(".");
#endif
        ...
        public void AggregatingCatalogAddAndRemoveChildren()
        {
            int changedCount = 0;
            int typesChanged = 0;
            EventHandler<ComposablePartCatalogChangedEventArgs> onChanged = delegate(object sender, ComposablePartCatalogChangedEventArgs e)
            {
                ++changedCount;
                typesChanged += e.ChangedDefinitions.Count();
            };
