...
/**
 * Parametric model for a LNN (Linear Non-Normal). This is a linear model
 ...
 */
public final class LnnPm implements TetradSerializable {
     ...
     * Copy constructor.
     */
    public LnnPm(LnnPm lnnPm) {
        this.graph = lnnPm.graph;
        this.nodes = new LinkedList<Node>(lnnPm.nodes);
        this.parameters = new LinkedList<Parameter>(lnnPm.parameters);
        this.variableNodes = new LinkedList<Node>(lnnPm.variableNodes);
        this.distributionTypes = new HashMap<Node, DistributionType>(lnnPm.distributionTypes);
        this.distributionParameters = new HashMap<Node, List<Parameter>>(lnnPm.distributionParameters);
        this.paramComparisons = new HashMap<ParameterPair, ParamComparison>(
                lnnPm.paramComparisons);
        this.bIndex = lnnPm.bIndex;
        this.dIndex = lnnPm.dIndex;
    }
     ...
     */
    public static LnnPm serializableInstance() {
        Dag dag = new Dag();
        GraphNode node1 = new GraphNode("X");
        dag.addNode(node1);
        return new LnnPm(Dag.serializableInstance());
    }
