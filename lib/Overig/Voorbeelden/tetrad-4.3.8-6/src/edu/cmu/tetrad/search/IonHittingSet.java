 ...
 * @author: Trevor Burns
 *
 * Provides a static method which implements a correct version of Reiter's hitting set
 * algorithm as described by Greiner, Smith, and Wilkerson in "A Correction to the
 * Algorithm in Reiter's Theory of Diagnosis" Artificial Intellegence 41 (1989) (see
 * for detailed specification). However, this is not a general implementation, it is
 * tailored for use in the ION search by dealing with GraphChange objects instead of
 * something more general.
 *
 * Varies mainly in that no explicit DAG is constructed, it is only implied by the
 * structure of the calls. This is because this implementation is concerned solely with
 * finding the hitting sets and once a node is processed its information is never again
 * accessed if one stores the necessary path of edge labels in newly constructed nodes.
 *
 * There is one exception to the above claim. If one follows the revised algorithm
 * strictly, in Enhancement Step 3 one does need information from previous levels.
 * The author decided this step was easier to code if one precomputes instead of
 * doing it on the fly, so this exception is inconsiquential.
 */
public class IonHittingSet {
    ...
    public static List<GraphChange> findHittingSet (List<HashSet<GraphChange>> Forig) {
        ...
        while(!currentLevel.isEmpty()) {
            ...
            if ( nodeRedundant(n, hittingSets) ) {} // do nothing
            else {
    ...
    /**
     * Implements one of the listed enhancements, checking if there is a node on the
     * next level which already has the new path. Returns true if a new node should be
     * created with that path and false if a new node is unnecessary.
     */
    ...
    /**
     * Implements one of the listed enhancements, checking if the path for the given
     * node n contains a known hitting set. Returns true if the node is redundant
     * and false if the node should be processed.
     */
    ...
    /**
     * Given a node and the List of HashSets returns the index of the first HashSet
     * in F that the node does not intersect or -1 if all are intersected
     */
     ...
     * List linearly, checking against all later HashSets in the List (both ways)
     * marking supersets with the addition of an empty GraphChange. Supersets are 
     * removed at the end in order to avoid the O(n) removal for each set individually. 
     * This effectively implements Enhancement step 3, just not the on-the-fly way 
     * specified.
     */
    ...
    /**
     * Structure for containing the pair of the path (potential hitting set)
     * and label (index of element of F which is first non-intersection). Allows
     * for label updating on the assumption that the node will be created with
     * its parent's label, to be used for a more efficient run of
     * findNextUCSigma
     */
    private static class HsNode {
        ...
        public void updateLabel (int label) { this.label = label; }

        public GraphChange getPath () { return path; }
