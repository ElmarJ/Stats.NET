...
import edu.cmu.tetradapp.model.LnnPmWrapper;
 ...
 */
public final class LnnPmEditor extends JPanel implements DelegatesEditing {
     ...
     */
    private LnnPmGraphicalEditor graphicalEditor;
     ...
     */
    public LnnPmEditor(LnnPm lnnPm) {
        if (lnnPm == null) {
...
//        add(graphicalEditor(), BorderLayout.CENTER);
     ...
     */
    public LnnPmEditor(LnnPmWrapper lnnImWrapper) {
        this(lnnImWrapper.getLnnPm());
    }
    ...
    private LnnPm getLnnPm() {
        return lnnPm;
    }

    private LnnPmGraphicalEditor graphicalEditor() {
        if (this.graphicalEditor == null) {
            this.graphicalEditor = new LnnPmGraphicalEditor(getLnnPm());
        }
        return this.graphicalEditor;
    }

    private JPanel exogenousTermEditor() {
        return new LnnExogenousTermEditor(lnnPm);
    }

    static class LnnPmGraphicalEditor extends JPanel {
         ...
         */
        public LnnPmGraphicalEditor(LnnPm lnnPm) {
            this.lnnPm = lnnPm;
            setLayout(new BorderLayout());
...
//                "Double click parameter names to edit parameters"));
        ...
        private void beginEdgeEdit(Edge edge) {
            ...
            ParameterEditor paramEditor = new ParameterEditor(parameter, lnnPm());
        ...
        private void resetNodeLabel(Node node) {
            ...
            DistributionType type = lnnPm().getDistributionType(varNode);
            List<Parameter> parameters = lnnPm().getDistributionParameters(varNode);
        ...
        private Parameter getNodeParameter(Node node) {
            Parameter parameter = lnnPm().getMeanParameter(node);

            if (parameter == null) {
                parameter = lnnPm().getVarianceParameter(node);
            }
            return parameter;
        }
        ...
        public Parameter getEdgeParameter(Edge edge) {
            if (Edges.isDirectedEdge(edge)) {
                return lnnPm().getCoefficientParameter(edge.getNode1(), edge.getNode2());
            } else if (Edges.isBidirectedEdge(edge)) {
                return lnnPm().getCovarianceParameter(edge.getNode1(), edge.getNode2());
            }
        ...
        private String getEquationOfNode(Node node) {
            ...
            SemGraph semGraph = lnnPm().getGraph();
...
//            Parameter edgeParam = lnnPm().getEdgeParameter(
//                    semGraph.getEdge(parentNode, node));
            ...
            eqn = eqn + " + " + lnnPm().getGraph().getExogenous(node);
        ...
        private final static class EdgeMouseListener extends MouseAdapter {
            private Edge edge;
            private LnnPmGraphicalEditor editor;

            public EdgeMouseListener(Edge edge, LnnPmGraphicalEditor editor) {
                this.edge = edge;
                this.editor = editor;
            }
            ...
            private LnnPmGraphicalEditor getEditor() {
                return editor;
            }
        ...
        private final static class NodeMouseListener extends MouseAdapter {
            ...
            public NodeMouseListener(Node node, LnnPmGraphicalEditor editor) {
                this.node = node;
                this.editor = editor;
            }
...
//    }
        ...
        private static final class ParameterEditor extends JPanel {
            ...
            public ParameterEditor(Parameter parameter, LnnPm lnnPm) {
                if (parameter == null) {
                    throw new NullPointerException();
                }
    ...
    static class LnnExogenousTermEditor extends JPanel {
        ...
        public LnnExogenousTermEditor(LnnPm lnnPm) {
            this.lnnPm = lnnPm;
        ...
        public void setup() {
            variableList = new JList(new VariableListModel(lnnPm.getVariableNodes()));
            ...
            for (Node node : lnnPm.getVariableNodes()) {
                DistributionEditor editor = new DistributionEditor(node, lnnPm);
                nodeEditors.put(node, editor);
            }

//            this.variableList = new JList(new VariableListModel(lnnPm.getVariableNodes()));
...
//            selectionBox.add(Box.createVerticalGlue());
    ...
    private static class DistributionEditor extends JPanel {
        ...
        public DistributionEditor(final Node node, final LnnPm lnnPm) {
            setLayout(new BorderLayout());
             ...
             comboBox.addActionListener(new ActionListener() {
                 public void actionPerformed(ActionEvent e) {
                     ...
                     if ("Normal".equals(selected)) {
                         lnnPm.setDistributionType(node, DistributionType.NORMAL);
                     }
                     else if ("Uniform".equals(selected)) {
                         lnnPm.setDistributionType(node, DistributionType.UNIFORM);
                     }
                 }
             });
