...
namespace System.ComponentModel.Composition
{
    ...
    public class DirectoryCatalogTests
    {
        ...
        public void ICompositionElementDisplayName_ShouldIncludeCatalogTypeNameAndDirectoryPath()
        {
            ...
            {
                var catalog = (ICompositionElement)CreateDirectoryCatalog(path);
        ...
        public void ICompositionElementDisplayName_ShouldIncludeDerivedCatalogTypeNameAndAssemblyFullName()
        {
            ...
            foreach (var path in paths)
            {
                var catalog = (ICompositionElement)new DerivedDirectoryCatalog(path);
        ...
        public void Parts_ShouldSetDefinitionOriginToCatalogItself()
        {
            using (var directory = CreateTemporaryDirectory())
            {
                var catalog = CreateDirectoryCatalog(directory.DirectoryPath);
                Assert.IsTrue(catalog.Parts.Count() > 0);
        ...
        public void Path_ValidPath_ShouldBeFine()
        {
            using (var directory = CreateTemporaryDirectory())
            {
                ...
                {
                    var cat = new DirectoryCatalog(e.Input);
        ...
        public void Refresh_AssemblyAdded_ShouldFireOnChanged()
        {
            using (var directory = CreateTemporaryDirectory())
            {
                ...
                cat.Changed += new EventHandler<ComposablePartCatalogChangedEventArgs>((o, e) =>
                    changedFired = true);
        ...
        public void GetExports()
        {
            using (var directory = CreateTemporaryDirectory())
            {
                ...
                IEnumerable<Tuple<ComposablePartDefinition, ExportDefinition>> expectedMathcingExports = catalog.Parts
                ...
                Assert.IsTrue(matchingExports.SequenceEqual(expectedMathcingExports));
        ...
        {
            public DerivedDirectoryCatalog(string path)
                : base(path)
            {
            }
        }
